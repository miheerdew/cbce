...
check <- function(res1, res2, 
                  ...
                  expect_differing_order=FALSE) {
 #Check extraction orders are the same
 ord1 <- res1$report[[4]]
 ord2 <- res2$report[[4]]
 
 if(expect_differing_order) {
   ex1 <- res1$extract_res
   ex2 <- res2$extract_res
 } else {
  ...
  if(expect_differing_indices) {
    ...
    expect_gt(length(ind)/min(length(ind1), length(ind2)), .9)
  } else {
...
test_that("ChiSq gives the same extraction", {
  ...
  #Indices differ because removed random ordering.
  check(res1b, res2, expect_differing_order = TRUE)
