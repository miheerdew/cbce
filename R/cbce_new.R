...
cbce2 <- function(X, Y, 
  ...
  extract <- function(indx) {
    ...
    if (length(B0$x)*length(B0$y) <= 1) {
      return(list("indx" = indx, dud = TRUE, success=FALSE))
    }
    
    # Initializing extraction loop
    
    chain <- c(hash(B0)) #The hashes of all sets visited during the extraction
  ...
  # Extracting
  list(extract_res=lapply(extractord, extract))
}